...
public class MessagesPanel extends JPanel implements DatabaseObserver {
    ...
    private final ArrayList<MessageDisplay> messages;
    ...
    public void on_message(Message message) {
        if (message.getFrom().equals(UserList.getCurrentUser().getUUID()) || message.getTo().equals(UserList.getCurrentUser().getUUID())) {
            this.addMessage(message, message.getTo().equals(this.database.getUUID()));
        }
    }
}
