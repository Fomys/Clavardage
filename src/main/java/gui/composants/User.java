...
public class User extends JPanel implements DatabaseObserver {
    ...
    private final Color blue = new Color(59, 130, 247);
    private final Color blancNickname = new Color(255, 255, 255);
    private final Color grisFonce = new Color(160, 160, 160);
    private final Color grisClair = new Color(230, 230, 230);
    private final Color grisBg = new Color(44, 43, 42);
    private final UUID uuid;
    private final String police = "Lucida Grande";
    private final int taillePolice = 13;
    ...
    public User(UUID uuid, Database database) throws IOException {
        this.uuid = uuid;
    ...
    private void initComponents() throws IOException {
        ...
        //---- nameDisplayed ----
        nameDisplayed.setText(this.database.getNicknameFor(this.uuid));
        ...
        nameDisplayed.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent arg0) {
                clickedOn();
            }
        ...
        dateLastMsg.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent arg0) {
                clickedOn();
            }
        ...
        add(lastMsg, new GridBagConstraints(1, 1, 2, 1, 0.0, 0.0,
                GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                new Insets(-10, 0, 0, 0), 0, 0));
        lastMsg.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent arg0) {
                clickedOn();
            }
        ...
        lblLed.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent arg0) {
                clickedOn();
            }
    ...
    public void on_message(Message message) {
        if (message.getFrom().equals(this.uuid) || message.getTo().equals(this.uuid)) {
    ...
    public void on_change_nickname(UUID uuid, String nickname) {
        ...
        if (this.uuid.equals(uuid)) {
            this.nameDisplayed.setText(nickname);
        }
    }
